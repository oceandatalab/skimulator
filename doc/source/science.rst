...
From the orbit nadir ground track the software generates a grid covering the SWOT swath over 1 satellite cycle. The longitude and latitude coordinates as well as the time are referenced for each grid point. A scheme of the SKIM geometry is presented on :ref:`Fig. 2 <Fig2>`.
The SKIM grid is stored by pass (e.g. 180 ascending passes and 180 descending passes for the sentinel-1a orbit). A pass is defined by an orbit starting at the lowest latitude for ascending track and at the highest latitude for descending track (+/- ?? for the considered SKIM orbits). The first pass starts at the first lowest latitude crossing in the input file, meaning that ascending passes are odd numbers and descending passes are even numbers.
...
============================================
Two main components of the nadir altimetry error budget are simulated : the altimeter noise and residual wet-tropospheric errors. For the altimeter noise, the noise follow a spectrum of error consistent with global estimates from the Jason-2 altimeter. The wet-tropospheric residual errors are generated using the simulated wet-tropospheric signal and radiometer beam convolution described in SWOT Simulator documentation.
